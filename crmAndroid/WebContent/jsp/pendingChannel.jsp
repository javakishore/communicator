...
<body>
	...
	<div class="title-bar"  style="    background-color:#E70028;margin: 0px;">
		...
		<h2 class="title" style="color:white">Pending Channel List</h2>
	</div>
	<div class="clear_both">&nbsp;</div>
	...
	<div class="container-fluid">
		<div class="row">
					...
					<c:choose>
						<c:when test="${user.userType=='admin'}">
							<ul class="main-nav" style="border:none;">
								...
								<li><a id="anchorfontcolor9" style="color:white;" class="active" href="/crmCmsAdmin/pendingchannel">Pending Channel</a></li>
			...
			<div class="col-md-10">
				<br/> <form:form class="form-horizontal" action="pendingchannels"
							...
							<c:choose>
								<c:when test="${!empty channelList}">
									<table border="0" class="table table-striped table-bordered ">
										<tr>
											<th width="40">Channel ID</th>
											<th width="60">Channel Name</th>
											 <th width="60">Activity</th>
											<th width="60">Action</th>
										</tr>
										<c:forEach items="${channelList}" var="channel">
										<input type="hidden" name="catid" value="${channel.channelId}"/>
											<tr>
												<td>${channel.channelId}</td>
												<td>${channel.channelName}</td>
												<c:set var = "action1" scope = "request" value = "${channel.action}"/>
												 <c:if test="${action1 == 0}">
							                    <td>ADD</td>
										         </c:if>
												 <c:if test="${action1 == 1}">
												...
												<td><a href="approvechannel?catid=+${channel.channelId}">Approve</a></td>
											</tr>
										</c:forEach>
									</table>
								</c:when>
